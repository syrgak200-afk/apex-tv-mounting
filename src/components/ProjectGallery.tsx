...
export function ProjectGallery() {
  ...
  return <section className="section container gallery-section" id="gallery" aria-labelledby="gallery-heading">
    ...
    <div className="project-masonry" aria-live="polite">{visibleProjects.map((project, index) => <button type="button" className="project-card" key={project.image} onClick={() => setActiveIndex(index)}><Image src={project.image} alt="" width={960} height={1280} sizes="(max-width: 719px) 100vw, (max-width: 1050px) 50vw, 33vw" loading="lazy" quality={76} /><span className="project-card-overlay"><small>{project.category}</small><strong>{project.title}</strong></span></button>)}</div>
